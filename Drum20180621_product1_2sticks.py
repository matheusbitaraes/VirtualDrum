...
y_2 = 0
enddraw = None
wasout_1 = []  # "drum stick was out in the last frame"
wasout_2 = []  # "drum stick was out in the last frame"
region = []
nameregion = []
...
while True:
    ...
    # RED COLOR
    lowerColor_2a = np.array([0, 100, 100])  # lower boundary of the hsv color
    upperColor_2a = np.array([10, 255, 255])  # upper boundary of the hsv color
    lowerColor_2b = np.array([170, 100, 100])  # lower boundary of the hsv color
    upperColor_2b = np.array([180, 255, 255])  # upper boundary of the hsv color

    start.append(timer())

    # masks creation
    mask_1 = cv2.inRange(hsv, lowerColor_1, upperColor_1)
    mask_2a = cv2.inRange(hsv, lowerColor_2a, upperColor_2a)
    mask_2b = cv2.inRange(hsv, lowerColor_2b, upperColor_2b)
    ...
    mask_2a = cv2.morphologyEx(mask_2a, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10, 10)))
    mask_2b = cv2.morphologyEx(mask_2b, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (10, 10)))
    ...
    contours_2a = cv2.findContours(mask_2a.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    contours_2b = cv2.findContours(mask_2a.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    contours_2 = contours_2a + contours_2b
    center = None
    ...
    if len(contours_1) > 0:
        ...
        # centroid
        c = sorted(contours_1, key=cv2.contourArea, reverse=True)
        ((x1, y1), radius) = cv2.minEnclosingCircle(c[0])
        M = cv2.moments(c[0])
        center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
        if len(contours_1) > 1:
            ((x_2, y_2), radius) = cv2.minEnclosingCircle(c[1])
        else:
            x_2 = 0
            y_2 = 0
